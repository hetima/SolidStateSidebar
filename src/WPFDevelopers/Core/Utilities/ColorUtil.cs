...
namespace WPFDevelopers.Utilities
{
    ...
    {
        public static Color ConvertHSLToColor(Color color, double h = double.NaN, double sl = double.NaN, double l = double.NaN)
        {
            var hsl = RgbToHSL(color);
            if (!double.IsNaN(h)) hsl.H = h;
            if (!double.IsNaN(sl)) hsl.S = sl;
            if (!double.IsNaN(l)) hsl.L = l;
            return HSLToRgb(hsl);
        }
        ...
        private static Color HSLToRgb(HSL hslColor)
        {
            ...
            double t1;
            if (hslColor.L < 0.5) t1 = hslColor.L * (1.0 + hslColor.S);
            else t1 = hslColor.L + hslColor.S - hslColor.L * hslColor.S;
        ...
        private static double SetColor(double t1, double t2, double t3)
        {
            if (t3 < 0) t3 += 1.0;
            if (t3 > 1) t3 -= 1.0;
            if (6.0 * t3 < 1) return t2 + (t1 - t2) * 6.0 * t3;
            if (2.0 * t3 < 1) return t1;
            if (3.0 * t3 < 2) return t2 + (t1 - t2) * (2.0 / 3.0 - t3) * 6.0;
            return t2;
        }
        ...
        public static HSL RgbToHSL(Color rgbColor)
        {
            ...
            var delta = max - min;
            if (max == min) { hslColor.H = 0; hslColor.S = 0; hslColor.L = max; return hslColor; }
            hslColor.L = (min + max) / 2;
            hslColor.S = hslColor.L < 0.5 ? delta / (max + min) : delta / (2.0 - max - min);
        ...
        public static void HsbFromColor(Color C, ref double H, ref double S, ref double B)
        {
            ...
            {
                if (DoubleUtil.AreClose(r, max)) hue = (g - b) / delta;
                else if (DoubleUtil.AreClose(g, max)) hue = 2 + (b - r) / delta;
                else if (DoubleUtil.AreClose(b, max)) hue = 4 + (r - g) / delta;
                hue = hue * 60;
                if (DoubleUtil.LessThan(hue, 0d)) hue += 360;
            }
            H = hue / 360d; S = saturation; B = brightness;
        }
        ...
        public static Color ColorFromHsb(double H, double S, double B)
        {
            ...
            if (DoubleUtil.IsZero(S)) { red = green = blue = B; }
            else
            {
