...
- [x] `src/Models/SettingsModel.cs` → `DockItem` / `ScreenItem` / `TextAlignItem` を `SettingItems.cs` へ分離。`SettingsModel` 本体の partial 分割は明確な境界がないため見送り。
- [x] `src/Core/Settings.cs` → `TextAlign` / `SectionHeaderStyle` enum を `MonitoringEnums.cs` へ移動（付随クラス `DateSetting` は Settings 固有のため残置）。
- [x] `src/Core/Converters.cs`（センサー値の単位変換 `iConverter`）→ `UnitConverters.cs` にリネームし、WPF の `src/Converters/Converters.cs` との混同を解消。
