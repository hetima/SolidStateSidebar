...
1. **バッキングフィールドが auto-property**
   `private string? _id { get; set; }` のように、バッキングフィールドを private auto-property で書くパターンがほぼ全クラスに存在する（`MonitorBase.cs`、`MetricBase.cs`、`MonitorPanel.cs`、`SidebarModel.cs`、`MonitorManager.cs`、`Sidebar.xaml.cs`、`Settings.cs` 等）。通常のフィールドにすべき。
...
3. **不要なファイナライザ**
   `~BaseMonitor()` `~BaseMetric()` `~MonitorPanel()` `~SidebarModel()` `~MonitorManager()` `~WindowMonitor()` `~ClaudeMonitor()` 等、アンマネージドリソースを直接保持しないクラスに一律ファイナライザが付いている。GC 負荷の無駄であり、Dispose パターンの誤用。
...
- [ ] `WindowMonitor.cs`: 空の `WindowItem_PropertyChanged` とその purchase/購読・解除コード（`Windows` setter 内、`Dispose` 内）を削除。
- [ ] `WindowMonitor.cs:19`: `_applications` の「//いらない」を解決する（`_applicationNames` 生成後は実際に `Update`/`UpdateFromHook` の null/Length チェックでしか使っていない → チェックをコンストラクタに移して削除、または保持するならコメント除去）。
- [ ] typo 修正: `pRocessicon` → `processIcon`、`lengthRetuen` → `lengthReturn`。
- [ ] `App.xaml.cs` `StartApp()`: 未使用の `_version` / `_vstring` を削除。
- [ ] `MonitorManager.UpdateWindowMonitor()` → `UpdateAllMonitors()` にリネーム（呼び出し元は `Update()` のみ）。
- [ ] `String` → `string` 統一（`MonitorPanel.cs` 等、出現箇所のみ）。
- [ ] 使われていない using の削除（自分が触ったファイルのみ）。
...
- [ ] `src/Core/ObservableObject.cs` を新設:
  ```csharp
  ...
  ```
  外部から `monitor.NotifyPropertyChanged(...)` を呼んでいる箇所があるため、必要なら `public` メソッドも残す（要 grep 確認）。
- [ ] 適用対象（1コミット = 1〜2クラス）:
...
- [ ] 同時に `private X _x { get; set; }` → `private X _x;` へ変換（auto-property バッキングの廃止）。
- [ ] 単純な転送プロパティは `SetProperty` で 3 行に圧縮。`IsAlert` のような副作用付き setter はロジックを変えずそのまま移植（副作用の整理は Phase 6）。

**検証:** ビルド + 実機でサイドバー全セクションの値更新・設定画面の双方向バインディング・アラート点滅を目視確認。バインディングエラーは VS 出力ウィンドウで確認。

**リスク:** バインディング切れ（プロパティ名 typo）。対策: リネームは IDE のリファクタリング機能のみ使用し、手書き文字列の `nameof` 化を優先。

---

## Phase 3: Dispose パターンとファイナライザの整理（半日）

- [ ] アンマネージドリソースを直接保持しないクラスからファイナライザを削除:
  `BaseMonitor` / `BaseMetric` / `OHMMonitorBase` / `MonitorPanel` / `MonitorManager` / `SidebarModel` / `WindowMonitor` / `ClaudeMonitor` / `CodexMonitor` / 各 `XxxUsageMetric`。
  （`Timer` / `HttpClient` / `CancellationTokenSource` は全てマネージドラッパーなので `Dispose(true)` 経路だけで足りる）
- [ ] `Dispose(bool)` の `_disposed` フラグ管理はそのまま維持。`GC.SuppressFinalize` はファイナライザ削除後は不要だが、`Dispose()` 定型として残しても害はない（基底に集約するなら削除）。
- [ ] P/Invoke ハンドルを扱う `Windows.cs` 内のクラス（`Hotkey` 等）は対象外（現状維持）。

**検証:** ビルド + リロード（設定変更→再生成）を数回繰り返してクラッシュ・リークがないこと。
...
- [ ] `src/Utilities/Windows.cs`（1672行）→ `src/Windows/` フォルダに分割:
  - `NativeMethods.cs`（P/Invoke 宣言を集約）
  - `OS.cs` / `WindowHelper.cs` / `ShowDesktop.cs` / `Devices.cs` / `Hotkey.cs`
  - `Monitor.cs`（`Monitor` / `WorkArea` / `RECT` / `MonitorExtensions` / `DockEdge`）
  - `DPIAwareWindow.cs` / `AppBarWindow.cs`
  - 名前空間 `SSS.Windows` は維持（参照箇所の変更ゼロで済む）
